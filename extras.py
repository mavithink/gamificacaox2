...
def renderizar(dados):
    ...
    # ACORDAR CEDO
    st.markdown("<h3 style='text-align: center;'>Recompensa Especial</h3>", unsafe_allow_html=True)
        ...
        with colunas_missoes[i]:
            st.markdown(f"""
                <div style='background-color: #FFFFFF; border: 1px solid #000000; padding: 10px; margin-bottom: 5px;'>
                    <strong style='font-size: 18px;'>{missao['desc']}</strong><br>
                    <span style='font-size: 14px; color: #666;'>Recompensa: +{missao['s']}$ | +{missao['x']}XP</span>
                </div>
    ...
    with col_sorte:
        st.markdown(f"""
            <div style="text-align: center; padding: 15px; border: 2px dashed #000000; background-color: #FFFFFF;">
                <h4 style="margin: 0; color: #000000;">Sua Sorte Diária ({dados['sorte_dia']['data']}):</h4>
